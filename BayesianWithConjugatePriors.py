...
# --- 5B. Bayesian Ridge Regression ---
print("\n--- 5B. Bayesian Ridge Regression ---")

# Create the Bayesian Ridge pipeline
# BayesianRidge estimates its parameters (alpha, lambda) automatically
br_pipeline = Pipeline(steps=[
    ('preprocessor', preprocessor),
    ('regressor', BayesianRidge(max_iter=300)) # n_iter is the main parameter, default is 300
])

# Train the Bayesian Ridge model on the full training data
print("Fitting BayesianRidge model...")
br_pipeline.fit(X_train_fe, y_train_log)

print("\nBayesianRidge Estimated Parameters:")
# Access the fitted regressor step to print estimated parameters
fitted_br = br_pipeline.named_steps['regressor']
print(f"  Estimated alpha (noise precision): {fitted_br.alpha_:.6f}")
print(f"  Estimated lambda (weight precision): {fitted_br.lambda_:.6f}")
print(f"  Estimated Intercept: {fitted_br.intercept_:.6f}") # <-- ADDED THIS LINE
print(f"  Estimated Coefficients (First 5): {fitted_br.coef_[:5]}...")

# Evaluate the Bayesian Ridge model on the training data
y_train_log_pred_br = br_pipeline.predict(X_train_fe)
y_train_pred_br = np.expm1(y_train_log_pred_br)
y_train_pred_br[y_train_pred_br < 0] = 0

rmse_train_br = root_mean_squared_error(y_train, y_train_pred_br)
r2_train_br = r2_score(y_train, y_train_pred_br)

print(f"\nBayesian Ridge Regression RMSE (Train): {rmse_train_br:,.2f}")
print(f"Bayesian Ridge Regression R² (Train): {r2_train_br:.4f}")

# --- Compare and Select Best Model ---
print("\n--- Model Comparison ---")
print(f"Baseline Linear Regression RMSE: {rmse_train_lr:,.2f} | R²: {r2_train_lr:.4f}")
print(f"Bayesian Ridge Regression RMSE: {rmse_train_br:,.2f} | R²: {r2_train_br:.4f}") # <-- CHANGED


final_model = br_pipeline # <-- CHANGED variable
print("✅ Using Bayesian Ridge Regression as final model.") # <-- CHANGED text
...
submission_filename = 'Bayesian_ConjugatePriors.csv' # Changed filename to reflect cleaning
