...
print("\n--- 5A. Baseline Linear Regression (on 85% split) ---")
...
# Train baseline model ON 85% SPLIT
print(f"Training baseline model on {X_train_fit.shape[0]} samples...")
lr_pipeline.fit(X_train_fit, y_train_fit_log)

# Evaluate baseline model on TRAINING SPLIT (for info only)
...
# --- 5B. Bayesian Optimization for Linear Regression (on 85% split) ---
print("\n--- 5B. Bayesian Optimization for Linear Regression (on 85% split) ---")
...
# Train BayesSearch ON 85% SPLIT
print(f"Running BayesSearchCV on {X_train_fit.shape[0]} samples...")
bayes_search.fit(X_train_fit, y_train_fit_log)

print("\nBest Bayesian Hyperparameters (from 85% split):")
print(bayes_search.best_params_)

# Get the best model found by the search
best_model_from_split = bayes_search.best_estimator_

# Evaluate optimized model on TRAINING SPLIT (for info only)
y_train_fit_log_pred_bayes = best_model_from_split.predict(X_train_fit)
y_train_fit_pred_bayes = np.expm1(y_train_fit_log_pred_bayes)
y_train_fit_pred_bayes[y_train_fit_pred_bayes < 0] = 0
rmse_train_fit_bayes = root_mean_squared_error(y_train_fit, y_train_fit_pred_bayes)
print(f"Bayesian Optimized RMSE (Train Split): {rmse_train_fit_bayes:,.2f}")

# Evaluate optimized model on VALIDATION SET
print(f"Evaluating Bayesian model on {X_val.shape[0]} validation samples...")
y_val_log_pred_bayes = best_model_from_split.predict(X_val)
...
# --- 6. Compare and Select Best Model (Based on Validation Score) ---
print("\n--- 6. Final Model Selection (Based on Validation Set) ---")
print(f"Baseline Linear Regression RMSE (Validation): {rmse_val_lr:,.2f}")
print(f"Bayesian Optimized Regression RMSE (Validation): {rmse_val_bayes:,.2f}")

if rmse_val_bayes < rmse_val_lr:
    print("\n✅ Bayesian-optimized model had better validation score. Re-training on 100% of data.")
    # Re-train the best estimator on the full dataset
    final_model = best_model_from_split.fit(X_train_fe, y_train_log)
else:
    print("\n✅ Baseline Linear Regression had better validation score. Re-training on 100% of data.")
    # Re-train the baseline pipeline on the full dataset
...
submission_filename = 'linear_Bayesian_split_select_retrain.csv' # Changed filename
