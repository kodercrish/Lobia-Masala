...
xgb_baseline_pipeline.fit(X_train_fit, y_train_fit_log)

# --- Evaluate baseline model ON VALIDATION SET ---
print("Evaluating baseline on 15% validation data...")
y_val_log_pred_xgb_base = xgb_baseline_pipeline.predict(X_val)
y_val_pred_xgb_base = np.expm1(y_val_log_pred_xgb_base)
y_val_pred_xgb_base[y_val_pred_xgb_base < 0] = 0

rmse_val_xgb_base = root_mean_squared_error(y_val, y_val_pred_xgb_base)
r2_val_xgb_base = r2_score(y_val, y_val_pred_xgb_base)

print(f"Baseline XGBoost RMSE (Validation): {rmse_val_xgb_base:,.2f}")
print(f"Baseline XGBoost R² (Validation): {r2_val_xgb_base:.4f}")


# --- 5B. Bayesian Optimization for XGBoost (on 85% Split) ---
...
search_space = {
    ...
    'regressor__max_depth': Integer(3, 10), # Trees are shallower than RF
...
print(f"Baseline XGBoost RMSE (Validation): {rmse_val_xgb_base:,.2f} | R²: {r2_val_xgb_base:.4f}")
...
if rmse_val_bayes < rmse_val_xgb_base:
    ...
    print("\n✅ Baseline XGBoost performed better. Retraining on FULL training data for submission...")
...
submission_filename = 'TestModels/xgboost_bayesian_submission.csv' # Changed filename
