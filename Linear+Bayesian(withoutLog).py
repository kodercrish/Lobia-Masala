## Linear + Bayesian Used Without Log Transformations in DataSet during Preprocessing 
...
# y_train_log = np.log1p(y_train)  <-- LOG TRANSFORM REMOVED AS REQUESTED
# We will now use y_train directly.
...
lr_pipeline.fit(X_train_fe, y_train) # <- Fit to y_train directly

# Evaluate baseline model
y_train_pred = lr_pipeline.predict(X_train_fe) # <- Predict directly
# y_train_pred = np.expm1(y_train_log_pred) # <- Removed expm1
...
bayes_search.fit(X_train_fe, y_train) # <- Fit to y_train directly
...
y_train_pred_bayes = best_model.predict(X_train_fe) # <- Predict directly
# y_train_pred_bayes = np.expm1(y_train_log_pred_bayes) # <- Removed expm1
...
y_test_pred = final_model.predict(X_test_fe) # <- Predict directly

# Reverse log-transformation
# y_test_pred = np.expm1(y_test_log_pred) # <- Removed expm1
...
submission_filename = 'linear&Bayesian_no_log.csv' # Changed filename to reflect cleaning
