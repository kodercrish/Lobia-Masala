...
from sklearn.model_selection import KFold, LeaveOneOut, cross_val_score # <-- ADD THIS
import pandas as pd
import numpy as np
...
# --- 5A. Baseline Linear Regression (with LOOCV) ---
print("\n--- 5A. Baseline Linear Regression (with LOOCV) ---")
...
# Define the LOOCV strategy
loocv = LeaveOneOut()

# Evaluate the baseline model using LOOCV
print("Running LOOCV on Baseline Linear Regression... (This may take a minute)")
# We ask for negative RMSE because cross_val_score maximizes a score.
# We then flip the sign back.
...
# The result is the average of all (e.g., 1450) RMSE scores
rmse_loocv_lr = -np.mean(scores_lr)
print(f"Baseline Linear Regression RMSE (from LOOCV): {rmse_loocv_lr:,.2f}")
...
# --- 5C. Evaluate Best Bayesian Model with LOOCV ---
print("\n--- 5C. Evaluating Best Bayesian Model with LOOCV ---")

print("Running LOOCV on Best Bayesian-Optimized Model... (This may take a minute)")
# Re-use the loocv object from 5A
scores_bayes = cross_val_score(
    best_model,  # <-- The model found by BayesSearchCV
    X_train_fe, 
    y_train_log, 
    cv=loocv, 
    scoring='neg_root_mean_squared_error',
...
rmse_loocv_bayes = -np.mean(scores_bayes)
print(f"Bayesian Optimized RMSE (from LOOCV): {rmse_loocv_bayes:,.2f}")
...
## --- Compare and Select Best Model (using LOOCV scores) ---
print("\n--- Model Comparison (based on LOOCV RMSE) ---")
print(f"Baseline Linear Regression LOOCV RMSE: {rmse_loocv_lr:,.2f}")
print(f"Bayesian Optimized LOOCV RMSE: {rmse_loocv_bayes:,.2f}")

if rmse_loocv_bayes < rmse_loocv_lr:
    final_model = best_model
    print("\n✅ Using Bayesian-optimized Linear Regression as final model.")
    print("   (This model is already trained on 100% of the data by BayesSearchCV)")
    print("\n--- Final Model Parameters ---")
    # Print the winning parameters
    print(final_model.named_steps['regressor'])
else:
    print("\n✅ Using baseline Linear Regression as final model.")
    print("   (Retraining baseline on 100% of the data for submission...)")
    # We must retrain the baseline, as cross_val_score does not return a model
